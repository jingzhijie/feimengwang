<template>
  <transition name="move">
    <div v-show="showdiscdetail" class="discdetailsbg">
      <div class="videoBox" v-if="mdetails.data.video !== ''">
        <div class="videotitle"><img  @click="hide" class="back" src="./back.png" alt=""></div>
        <video ref="video" :src="baseurl + mdetails.data.video" controls="controls"></video>
      </div>
      <div class="downbg" v-bind:class="[mdetails.data.video!== ''? yesClass : '',noClass]">
        <div v-show="mdetails.data.video === ''" class="title"><img  @click="hide" class="back" src="./back.png" alt=""></div>
        <div v-show="mdetails.data.video === ''" class="disctitle">{{mdetails.data.title}}</div>
        <div v-show="mdetails.data.video !== ''" class="disctitlev">{{mdetails.data.title}}</div>
        <div class="summary">
          <img class="author_tx" :src="baseurl + mdetails.data.photo" alt="" width="54">
          <div class="author_name">
            <dl class="">
              <dt>作者：{{mdetails.data.name}}</dt>
              <dd>{{mdetails.data.school_name}}</dd>
            </dl>
          </div>
          <div class="clear"></div>
          <div class="author_intro">
            <div class="intro_inner"><p v-for="(item,index) in mdetails.data.info" :key = 'index'>{{item}}</p></div>
            <div class="seemore" @click="showIntro">查看更多</div>
            <div class="type_intro">
              <div class="laiyuan">来源：作品上传</div>
              <div class="leibie">作品类别：绘画视频{{mdetails.data.category}}</div>
              <div class="clear"></div>
            </div>
          </div>
        </div>
        <div class="pictitle">代表图片</div>
        <div class="picswiper" v-if="mdetails.data.thumbs">
          <swiper :options="swiperOption">
            <swiper-slide v-for="(item,index) in mdetails.data.thumbs" :key = 'index'><img :src="baseurl + item"  alt=""></swiper-slide>
          </swiper>
        ...
        <div class="commentBox">
          <div class="comment">
            <h3>评论</h3>
            <div class="commentlist" v-for="(item,index) in mdetails.message" :key = 'index'>
              <div class="ask">
                <div class="top" @click="respondA(item.username,item.who_message,item.id,mdetails.data.id)">
              ...
              <div class="answer" v-for="(v,k) in item.sub_message" :key = 'k'>
                <div class="top">
                  ...
                  <div class="tname" @click="respondA(v.username,v.who_message,v.pid,mdetails.data.id)">{{v.is_tutor === '1' ? v.username  + "（老师）" : v.username}}</div>
                  <div class="icon"><img src="./iconsj.png" alt="" width="5"></div>
                  <div class="name" @click="respondA(v.message_who_name,v.message_who,v.pid,mdetails.data.id)">{{v.message_who_name}}</div>
        ...
        <div class="addcomment">
          <img class="tx" :src="baseurl + mdetails.LoginUserPhoto" alt="" width="26">
      ...
      <m-intro ref="intro" :introdata="mdetails.data"></m-intro>
    </div>
  </transition>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        touxiang: '/static/img/touxiangl.png',
        whoMessage: '',
        wid: this.mdetails.data.id,
        pid: 0,
        huifu: '',
        level: 1,
        whoName: '',
    ...
    methods: {
      ...
      sendMsg() {
        let that = this
        that.wid = that.mdetails.data.id
